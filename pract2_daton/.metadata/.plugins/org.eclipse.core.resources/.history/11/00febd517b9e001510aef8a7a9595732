...
public class main {
	...
	private static void aleatoria(int nodos){
		String nombreArchivo = "C:\\TP\\pr2\\nodosAleatoria.csv", fase = "";
		double p;
	...
	private static void aristasAleatoria(int nodos, double p, String fase){
		String texto = "Source;Target;Type\n", nombreCarpeta = "C:\\TP\\pr2\\aristas" + fase;
	 ...
	 /****************************
		RED LIBRE DE ESCALA
		Utilizará el mismo csv de nodos generado por la aleatoria
	  */
	private static void libreDeEscala(int nodos){
		String nombreCarpeta = "C:\\hlocal\\TP\\pr2\\aristasLibreEscala";
		File carpeta = new File(nombreCarpeta);
		carpeta.mkdir();
		...
		while (t < T){
			//for (int m = 5; m <= N; m++){
			k++;//para todos los nuevos nodos que se tienen que añadir	
			//genero m enlaces (3)
				for (int n = 0; n < M; n++){	
					...
					//recorro la lista de aristas para ver si ya está esa arista(tupla)
